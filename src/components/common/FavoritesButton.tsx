...
const FavoritesButton = ({ article, className, text, onChange }: FavoritesButtonProps) => {
  ...
  const handleFavorites = async () => {
    ...
    try {
      const data = favorited
        ? await fetchWrapper<ArticleItem>(`/articles/${slug}/favorite`, "DELETE")
  ...
  return (
    <button
      ...
      disabled={loading}
      className={`${className} ${favorited ? "btn-primary" : "btn-outline-primary"}`}
    >
